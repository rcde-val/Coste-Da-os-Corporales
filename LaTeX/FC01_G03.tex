...
\section{Análisis metodológico escogido y resultados}
    ...
    \subsection{Modelización seleccionada y objetivos a alcanzar}
        \subsubsection{Número de siniestros}
        	A continuación, desarrollaremos los distintos modelos estudiados a lo largo de la asignatura 'Modelización estadística', indicando en cada caso las razones por las que no resultan aplicables cuando corresponda.
        	...
        	\paragraph{Clúster jerárquico aglomerativo} Pendiente
        	...
        	\paragraph{Clúster jerárquico divisivo}
	        	...
	        	\begin{itemize} 
	        		...
	        		\item \textbf{Tamaño de Nodos:} Se requiere un mínimo de 3000 observaciones para intentar una división ($min_split$) y 1500 para un nodo terminal ($min_bucket$). 
			...
			\paragraph{Análisis PCA} Pendiente
        	
        \subsubsection{Costo del siniestro}
	        En las siguientes secciones presentaremos los diferentes modelos abordados durante la asignatura 'Cuantificación de riesgos'.
	                
        	\paragraph{Modelización no paramétrica} David
        	      	
        	\paragraph{Modelización paramétrica} Sergio

			\paragraph{Teoría de valores extremos} A CONTINUACIÓN SE DETALLAN LAS EVIDENCIAS ESTADÍSTICAS Y GRÁFICAS: (VA ALGUNA IMAGEN??)
			
				El análisis de la variable de coste de daños corporales, fundamentado en la Teoría de Valores Extremos (EVT), confirma la presencia de una distribución de cola pesada (heavy-tailed distribution). A continuación se detallan las evidencias estadísticas y gráficas:
				
				Después de haber seleccionado únicamente los casos positivos de la variable \textit{InjuryAmount} y haberla transformado dividiéndola por 1.000. Los estadísticos básicos revelan una asimetría extrema y una curtosis muy elevada. Existe una discrepancia masiva entre la tendencia central (Media $\approx 15.75$) y el valor máximo ($19,792.8$). El tercer cuartil ($Q_3 \approx 9.13$) es insignificante comparado con el máximo, lo que indica que la gran mayoría de la densidad de probabilidad se concentra cerca de cero, pero el riesgo (la varianza y la media en menor medida) está dominado por eventos extremos poco frecuentes. Para validar el comportamiento de la cola de la distribución y confirmar la selección del umbral, se han analizado las siguientes herramientas visuales:
			
				\begin{itemize}
					\item \textbf{Mean Excess Plot (Gráfico de Exceso Medio):} Muestra una clara tendencia lineal con pendiente positiva a medida que aumenta el umbral. En el contexto de la Teoría de Valores Extremos (EVT), esta pendiente positiva es una señal característica de una distribución de cola pesada. Esto confirma que el parámetro de forma es estrictamente positivo, descartando distribuciones de cola ligera o exponencial.
					\item \textbf{Hill Plot:} Representa las estimaciones del índice de cola en función de los estadísticos de orden. Se observa una zona de estabilidad (indicada por la línea discontinua horizontal) donde el estimador converge a un valor aproximado de $0.9$. La línea vertical azul confirma que el umbral elegido ($u=21$) se encuentra dentro de esta región estable, validando la robustez de la estimación.
					\item \textbf{CV-Plot (Gráfico de Coeficiente de Variación):} 
					Esta herramienta complementaria analiza la dispersión de los excesos sobre el umbral. El comportamiento observado respalda la elección del umbral, mostrando consistencia con los resultados del Hill Plot y asegurando que la varianza de la cola se comporta según lo esperado para una GPD.
				\end{itemize}
				
				Basándonos en la estabilidad de los gráficos de diagnóstico, se fijó un umbral de $u \approx 21$. Este corte aisla las 1616 observaciones más extremas de la muestra, que representan aproximadamente el $10\%$ superior de la distribución de pérdidas ($p < u \approx 0.90$).
				
				Se ajustó la Distribución Generalizada de Pareto (GPD) a estos excesos mediante el método de Máxima Verosimilitud (MLE). Los parámetros resultantes confirman la severidad de la cola, parámetro de forma ($\tilde{\zeta}$): $0.8831$ (Error estándar: $0.0456$) y parámetro de escala ($\psi$): $19.2994$ (Error estándar: $0.9092$).
				
				El valor de $\tilde{\zeta} \approx 0.88$ es positivo y significativamente distinto de cero (considerando su error estándar bajo), lo que ratifica la presencia de una cola pesada.
				
				Con los parámetros estimados, se procedió al cálculo del Capital Económico necesario para cubrir el riesgo al nivel de confianza del $99.5\%$ ($q=0.995$). Se compararon los resultados del enfoque de Pareto simple (Hill) frente al enfoque POT-GPD:
				...
				% Modelización seleccionada y objetivos a alcanzar: Costo del siniestro - Teoría de valores extremos
					\begin{table}[H]
						...
						\begin{tabular}{lcc}
							...
							\midrule
								Pareto simple & $\hat{\xi}^{Hill} \approx 1.13$ & 284,32 \\
								GPD (POT) & $\tilde{\zeta} \approx 0.88$, $\psi \approx 19.30$ & 285,77 \\
							\bottomrule
						\end{tabular}
					\end{table}
								
				Como se observa en la Tabla, ambos modelos convergen hacia un riesgo extremo muy similar, situando el VaR en torno a las 285 unidades monetarias. 
				
				Específicamente, el modelo GPD estima un $VaR_{99.5\%} \approx 285.77$. Esto significa que, con una probabilidad del $0.5\%$, la entidad podría enfrentar pérdidas que superen este monto. Este valor es extremadamente alto en comparación con la media de los datos, lo que subraya que el riesgo principal de la cartera reside en estos eventos de baja frecuencia pero alto impacto.
