...
\section{Análisis metodológico escogido}
    El objetivo inicial es comprender la estructura del conjunto de datos y familiarizarnos con las variables disponibles, ya que esto constituye la base para cualquier análisis exploratorio. En el Cuadro 01 se presenta una descripción resumida de las variables correspondientes a una cartera de una aseguradora en Francia, que incluyen características del vehículo, del conductor, de la póliza y los montos asociados a los reclamos, entre otros.
    ...
    \subsection{Análisis descriptivo univariado y bivariado de la base de datos}
        La variable ClaimNb indica el número de reclamos por póliza. La distribución está fuertemente concentrada en cero (96,09\%), con pocos casos de 1 reclamo (3,54\%) y valores mayores extremadamente raros. Presenta una alta asimetría positiva (5,78), lo que significa que la mayoría de los valores se encuentran en el extremo inferior (cero) y existe una cola larga hacia la derecha debido a unos pocos casos con múltiples reclamos. Además, la curtosis es muy elevada (38,79), lo que indica una distribución con un pico muy pronunciado en torno a cero y colas pesadas, reflejando la presencia de valores extremos que, aunque escasos, están muy alejados del centro y afectan la forma de la distribución.
...
% Análisis descriptivo univariado (Variables discretas y contínuas)
	\begin{table}[H]
		\centering
		\caption{Análisis descriptivo univariado - variables discretas y continuas}
		\begin{tabularx}{\textwidth}{llllll}
			\toprule
			\textbf{Concepto} & \textbf{Claim Nb} & \textbf{Exposure} & \textbf{VehAge} & \textbf{DrivAge} & \textbf{Density} \\
			\midrule
			Media & 0.04 & - & - & - & - \\
			Mediana & 0 & - & - & - & - \\
			Mínimo & 0 & - & - & - & - \\
	...
	\begin{table}[H]\ContinuedFloat
		\centering
		\caption{Análisis descriptivo univariado - variables discretas y continuas (cont.)}
