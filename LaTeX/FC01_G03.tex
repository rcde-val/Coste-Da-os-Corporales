...
\section{Análisis metodológico escogido}
    ...
    PropertyAmount & 3 & Costo por daños materiales. \\
    \bottomrule
    \end{tabularx}
    \vspace{0.5em}
    \raggedright
    ...
    \subsection{Análisis descriptivo univariado y bivariado de la base de datos}
    	...
    	Se realiza un análisis descriptivo de las variables numéricas considerando medidas de tendencia central, dispersión y forma (\hyperref[Anexo01]{\textcolor{blue}{Anexos - Cuadro 2}}). Adicionalmente, para las variables Exposure, CarAge, DriverAge y Density se aplica una técnica de segmentación mediante el método de k-means (\hyperref[Anexo02]{\textcolor{blue}{Anexos - Figura 2}}). Esta metodología permite clasificar los datos en conjuntos homogéneos, minimizando la variabilidad interna dentro de cada grupo y maximizando la diferencia entre ellos. Para determinar el número óptimo de clusters se utilizó el método del codo. Concluido este análisis, se procede al estudio de las variables categóricas, examinando su distribución de frecuencias y representaciones gráficas para identificar patrones relevantes. Los aspectos más significativos se detallan a continuación:
    		\begin{itemize}
    			...
    			\item \textbf{Costo total del reclamo (ClaimAmount):} presenta una distribución altamente desbalanceada y con valores extremos. El valor promedio es de 832,57 euros, mientras que la mediana es cero, lo que indica que más del 50\% de los registros no tienen reclamo asociado. El mínimo también es cero, confirmando la existencia de pólizas sin siniestro, mientras que el máximo alcanza los 20.368.330 euros, evidenciando la presencia de valores extremadamente altos. La desviación estándar, de 41.847 euros, refleja una gran dispersión en los datos. Además, la asimetría es de 375,9 y la curtosis de 166.362,6, lo que indica una distribución extremadamente sesgada hacia la derecha, con una concentración muy alta en valores bajos y la presencia de outliers. Para complementar este análisis, se elaboraron dos histogramas (\hyperref[Anexo03]{\textcolor{blue}{Anexos - Figura 3}}), considerando únicamente las pólizas con al menos un siniestro, dado que el 96,1\% de las pólizas no registraron ningún reclamo y, por tanto, un histograma completo concentraría la masa de probabilidad en cero, ocultando el comportamiento del resto:
    				\begin{itemize}
    					\item El primer histograma corresponde al 99 \% de las pólizas con siniestros y revela que la gran mayoría de los reclamos son de bajo coste: el 96,9 \% no supera los 50.000 euros, mientras que solo el 2,4\% se encuentra entre 50.000 y 100.000 euros, y menos del 1 \% supera los 100.000 euros. El coste promedio dentro de este grupo es de 13.560 euros, lo que confirma que los siniestros de alto importe son excepcionales. El gráfico refuerza esta conclusión, mostrando una barra dominante en el primer intervalo (0–50 miles de euros) y frecuencias marginales en los tramos superiores, lo que evidencia una concentración extrema en valores bajos. 
    					\item El histograma del 1 \% de las pólizas con siniestros muestra que, aunque este grupo corresponde a los casos más costosos, la concentración sigue siendo muy marcada en el primer intervalo: el 98,8 \% de estos reclamos extremos se sitúan entre 161.500 y 5.000.000 euros, con un coste promedio en este tramo de 577.685 euros. Esto indica que, incluso dentro de la cola, la mayoría de los siniestros se ubican en rangos elevados pero moderados en comparación con los máximos observados, aunque siguen siendo significativamente superiores al promedio general. Los intervalos superiores, que incluyen reclamos entre 10 y 15 millones y entre 20 y 25 millones de euros, apenas representan el 0,6 \% cada uno, lo que confirma que los valores más altos son casos excepcionales con un impacto potencial muy significativo en la compañía aseguradora.
    				\end{itemize}
    			\item \textbf{Costo de compensación por daños corporales (InjuryAmount):} Presenta una alta concentración en valores nulos y una cola prolongada con pocos casos de gran magnitud. El promedio es 615,9 euros, mientras que la mediana y el mínimo son cero; el máximo alcanza 19.972.821 euros, reflejando reclamos excepcionales. La dispersión es elevada (desviación estándar: 40.867 euros) y los indicadores de forma son extremos: asimetría 375,25 y curtosis 165.244,5, lo que evidencia un fuerte sesgo a la derecha. Para evitar que los ceros oculten el patrón real, se elaboraron dos histogramas considerando solo pólizas con siniestros, confirmando la concentración en valores bajos y la existencia de una cola con pocos casos de gran coste. Ambos gráficos se incluyen en los anexos. (\hyperref[Anexo02]{\textcolor{blue}{Anexos - Figura 3}}).
    			\item \textbf{Costo por daños materiales (PropertyAmount):} La distribución muestra una fuerte concentración en valores nulos y casos positivos menos extremos que en \textit{\textbf{InjuryAmount}}. El promedio es 216,7 euros, la mediana y el mínimo son cero, y el máximo llega a 575.508,8 euros, lo que indica reclamos significativos pero no desproporcionados. La dispersión es alta (desviación estándar: 1.554 euros) y la asimetría (129,54) y curtosis (45.682,36) confirman un sesgo marcado hacia la derecha. Para evitar que los ceros oculten el patrón real, se elaboraron dos histogramas: el primero concentra el 99 \% de los casos por debajo de 12.000 euros, destacando los rangos de 8.000-10.000 euros (26,3 \%), 4.000-6.0000 euros (23,7 \%) y  2.000-4.000 euros (22,3 \%). El segundo muestra el 1 \% restante, casi todo entre 10.400 y 100.000 euros, con algunos reclamos aislados de mayor coste. Aunque existen siniestros elevados, la mayoría se sitúa en valores moderados. Ambos gráficos se incluyen en los anexos. (\hyperref[Anexo02]{\textcolor{blue}{Anexos - Figura 3}}).
    		\end{itemize}
		Tras el análisis detallado de las variables numéricas, se continúa con el estudio de las variables categóricas, examinando su distribución de frecuencias y representación gráfica para identificar patrones relevantes:
			\begin{itemize}
				\item \textbf{Potencia del coche (Power):}
				\item \textbf{Marca del vehículo (VehBrand):}
				\item \textbf{Tipo de combustible (VehGas):}
...
% Análisis descriptivo univariado (Variables discretas y contínuas): Tabla estadísticos
	...
	\begin{figure}[htbp]
		\centering
		\caption{Análisis descriptivo univariado - Técnica de segmentación}
		% Primera fila
...
\begin{figure}[htbp]
	\centering
	\caption{Análisis descriptivo univariado: Histogramas}
	% Primera fila
	\begin{subfigure}[b]{0.4\textwidth}
